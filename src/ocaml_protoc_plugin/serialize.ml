...
let rec write : type a. a compound -> Writer.t -> a -> unit = function
  ...
  | Repeated (index, spec, Packed) when is_scalar spec ->
...
module Test = struct
  let test () =
    ...
    let (_ : bool) =
      signed_varint 2147483647L = Varint 4294967294L
      || failwith "signed_varint 2147483647L"
    in
    let (_ : bool) =
      signed_varint (-2147483648L) = Varint 4294967295L
      || failwith "signed_varint -2147483648L"
    in
    ()
  ;;
end
