...
let read_raw_varint t =
  ...
  in
  inner [] >>| List.fold_left ~init:0L ~f:(fun acc c -> (acc lsl 7) + c)
;;
...
let to_list : t -> (int * Field.t) list Result.t =
 fun t ->
  let rec inner acc =
    match has_more t with
    | true -> read_field t >>= fun v -> inner (v :: acc)
