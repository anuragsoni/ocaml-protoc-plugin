...
let error_wrong_field str field = raise (Common.Wrong_field_type (str, field))
let error_illegal_value str field = raise (Common.Illegal_value (str, field))
let error_required_field_missing () = raise Common.Required_field_missing
...
let rec type_of_spec : type a. a spec -> 'b * a decoder =
  ...
  let int_of_int64 spec =
    let tpe, f = type_of_spec spec in
    let f field = Int64.to_int (f field) in
    tpe, f
  in
  ...
  | Bytes ->
    ...
    , (function
      | Field.Length_delimited { offset; length; data } ->
        String.sub ~pos:offset ~len:length data |> Bytes.unsafe_of_string
      | field -> error_wrong_field "string" field) )
  | Message from_proto ->
    ( `Length_delimited
    , (function
...
let sentinal : type a. a compound -> (int * unit decoder) list * a sentinal = function
  ...
  | Basic (index, spec, Required) ->
    ...
    let get () =
      ...
      | None -> error_required_field_missing ()
    in
    let read field = v := Some (read field) in
    [ index, read ], get
  | Basic (index, spec, default) ->
    ...
    let default =
      ...
      | Required | Proto3 -> default_of_field_type field_type |> read
    in
    let v = ref default in
    let get () = !v in
    let read field =
      let value = read field in
      v := value
    in
    [ index, read ], get
  ...
  | Repeated (index, spec, _) ->
    let read_field = function
      ...
      | `Varint -> Some (fun reader -> Reader.read_varint reader)
      | `Fixed_64_bit -> Some (fun reader -> Reader.read_fixed64 reader)
      | `Fixed_32_bit -> Some (fun reader -> Reader.read_fixed32 reader)
    in
    let rec read_repeated reader decode read_f =
      match Reader.has_more reader with
      | false -> ()
      | true ->
        let field = decode reader in
        read_f field;
    ...
    let rec read field =
      match field, read_field field_type with
      | (Field.Length_delimited _ as field), None ->
        let v' = read_type field in
        v := v' :: !v
    ...
    [ index, read ], get
  | Oneof oneofs ->
    let make_reader : a ref -> a oneof -> int * unit decoder =
     fun v (Oneof_elem (index, spec, constr)) ->
      let _, read = type_of_spec spec in
      let read field =
        let value = read field in
        v := constr value
...
module Map = struct
  ...
  let of_alist_exn l =
    List.fold_left
      ~init:empty
      ~f:(fun acc (k, v) ->
        if mem k acc then invalid_arg "Duplicate keys in list" else add k v acc)
      l
  ;;
end
...
let read_fields_array extension_ranges max_index reader_list =
  ...
  let rec read reader =
    ...
    | true ->
      (match Reader.read_field reader with
      | index, field when index <= max_index ->
        readers.(index) index field;
        read reader
      | index, field ->
        default index field;
        read reader)
...
let deserialize
    : type constr t.
      (int * int) list
      -> (constr, t) compound_list
      -> ((int * Field.t) list -> constr)
      -> Reader.t
      -> t
  =
 fun extension_ranges spec constr ->
  let max_index =
    let rec inner : type a b. int -> (a, b) compound_list -> int =
     fun acc -> function
      | Cons (Oneof oneofs, rest) ->
        let rec max_elt : type c. int -> c oneof list -> int =
         fun acc -> function
        ...
        inner acc rest
      | Cons (Basic (idx, _, _), rest) -> inner (max acc idx) rest
      | Cons (Basic_opt (idx, _), rest) -> inner (max acc idx) rest
      | Cons (Repeated (idx, _, _), rest) -> inner (max acc idx) rest
      | Nil -> acc
    in
    inner 0 spec
  in
  (* For even better optimization, the first pass could assume that all fields are written
     (if at all) in the same order as the spec. If we reach the end of the reader list, we
     revert to use read_fields_array or read_fields_map.

     Even even better, we could read opportunistically, and apply to the constructor as
     soon as we find the element. This requires that fields are collected into lists. Also
     Oneof must be in the correct order, and requires special handling (But I dont see
     that as a problem). This solution avoids the need of sentinals, and has O(n), where n
     is the number of fields. If passing fails, we start over and apply standard
     readfields. *)
  let read_fields =
    match max_index < 1024 with
    | true -> read_fields_array extension_ranges max_index
    | false -> read_fields_map extension_ranges
  in
  let rec apply : type constr t. constr -> (constr, t) sentinal_list -> t =
   fun constr -> function
    | SCons (sentinal, rest) ->
      let v = sentinal () in
      apply (constr v) rest
  ...
  let rec make_sentinals
      : type a b. (a, b) compound_list -> (a, b) sentinal_list * (int * unit decoder) list
    = function
    | Cons (spec, rest) ->
  ...
  fun reader ->
    ...
    let extensions = read_fields reader_list reader in
    apply (constr extensions) sentinals
;;
