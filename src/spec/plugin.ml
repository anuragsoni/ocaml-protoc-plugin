...
(************************************************)
(* Source: google/protobuf/compiler/plugin.proto Syntax: proto2 Parameters: debug=false
  ...
  module Protobuf = struct
    module Compiler = struct
      module rec Version : sig
        ...
        let to_proto =
          let apply ~f:f' { major; minor; patch; suffix } =
            f' [] major minor patch suffix
          in
          let spec =
            Runtime'.Serialize.C.(
        ...
        let from_proto =
          let constructor _extensions major minor patch suffix =
      ...
      and CodeGeneratorRequest : sig
        ...
        type t =
          ...
          ; proto_file :
              Imported'modules.Descriptor.Google.Protobuf.FileDescriptorProto.t list
        ...
        let make ?file_to_generate ?parameter ?proto_file ?compiler_version () =
          let file_to_generate =
            match file_to_generate with
            | Some v -> v
            | None -> []
          in
          let proto_file =
        ...
        let to_proto =
          let apply ~f:f' { file_to_generate; parameter; proto_file; compiler_version } =
            f' [] file_to_generate parameter proto_file compiler_version
          in
          let spec =
            Runtime'.Serialize.C.(
              repeated (1, string, not_packed)
              ^:: basic_opt (2, string)
              ^:: repeated
                    ( 15
                    , message (fun t ->
                        Imported'modules.Descriptor.Google.Protobuf.FileDescriptorProto
                        .to_proto
                          t)
                    , not_packed )
              ^:: basic_opt (3, message (fun t -> Version.to_proto t))
              ^:: nil)
          in
          let serialize = Runtime'.Serialize.serialize [] spec in
          fun t -> apply ~f:serialize t
              ...
              ^:: repeated
                    ...
                    , message (fun t ->
                        ...
                        .from_proto
                          t)
                    , not_packed )
              ^:: basic_opt (3, message (fun t -> Version.from_proto t))
              ^:: nil)
          in
          let deserialize = Runtime'.Deserialize.deserialize [] spec constructor in
          fun writer -> deserialize writer
      ...
      and CodeGeneratorResponse : sig
        ...
        let make ?error ?supported_features ?file () =
          let file =
            match file with
            | Some v -> v
            | None -> []
        ...
        let to_proto =
          let apply ~f:f' { error; supported_features; file } =
            f' [] error supported_features file
          in
          let spec =
            Runtime'.Serialize.C.(
              basic_opt (1, string)
              ^:: basic_opt (2, uint64_int)
              ^:: repeated
                    ( 15
                    , message (fun t -> CodeGeneratorResponse.File.to_proto t)
                    , not_packed )
              ^:: nil)
          in
          let serialize = Runtime'.Serialize.serialize [] spec in
        ...
        let from_proto =
          let constructor _extensions error supported_features file =
            { error; supported_features; file }
          in
          let spec =
            Runtime'.Deserialize.C.(
              ...
              ^:: repeated
                    ( 15
                    , message (fun t -> CodeGeneratorResponse.File.from_proto t)
                    , not_packed )
              ^:: nil)
          in
          let deserialize = Runtime'.Deserialize.deserialize [] spec constructor in
