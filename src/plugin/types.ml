...
(* This module is a bit elaborate. The idea is to construct the actual types needed in the
   spec module.

   This will ensure that the plugin will only construct valid types, so that chnges to the
   spec will require changes here also. *)
...
let make_default : type a. a spec -> string -> a = function
  ...
  | Enum _ ->
    fun x -> failwith (sprintf "Defaults for enums cannot be handled here: %s" x)
...
let default_of_spec : type a. a spec -> string =
 fun spec ->
  match spec with
...
let string_of_spec : type a. [ `Deserialize | `Serialize ] -> a spec -> string =
 fun dir spec ->
  match dir, spec with
  | _, Double -> "double"
  | _, Float -> "float"
...
let spec_of_enum ~scope type_name default =
  ...
  let default =
    Option.map
      ~f:(fun default -> Scope.get_scoped_name ~postfix:default scope type_name)
      default
  in
...
let string_of_proto_type : type a. a spec -> a proto_type -> string =
 fun spec -> function
  | Proto3 -> "proto3"
...
let c_of_compound : type a. string -> a compound -> c =
 fun name compound ->
  match compound with
  | Basic (index, spec, proto_type) ->
    let deserialize_spec =
      sprintf
        "basic (%d, %s, %s)"
        index
        (string_of_spec `Deserialize spec)
        (string_of_proto_type spec proto_type)
    in
    let serialize_spec =
      sprintf
        "basic (%d, %s, %s)"
        index
        (string_of_spec `Serialize spec)
        (string_of_proto_type spec proto_type)
    in
    let modifier =
  ...
  | Basic_opt (index, spec) ->
    let deserialize_spec =
      sprintf "basic_opt (%d, %s)" index (string_of_spec `Deserialize spec)
    in
    let serialize_spec =
      sprintf "basic_opt (%d, %s)" index (string_of_spec `Serialize spec)
    in
...
let c_of_field ~params ~syntax ~scope field =
  ...
  | _, { label = Some Label.LABEL_OPTIONAL; type' = Some TYPE_MESSAGE; type_name; _ } ->
    ...
    Basic_opt (number, spec) |> c_of_compound name
  (* Required message *)
  | ( `Proto2
    , { label = Some Label.LABEL_REQUIRED; type' = Some TYPE_MESSAGE; type_name; _ } ) ->
    let spec = spec_of_message ~scope type_name in
    Basic (number, spec, Required) |> c_of_compound name
  ...
  | ( `Proto2
    ...
    Basic (number, spec, Proto2 (Some default)) |> c_of_compound name
  ...
  | ( `Proto2
    ...
    Basic (number, spec, Proto2 default) |> c_of_compound name
  ...
  | `Proto3, { label = Some Label.LABEL_OPTIONAL; type' = Some type'; type_name; _ } ->
    let (Espec spec) = spec_of_type ~params ~scope type_name None type' in
    Basic (number, spec, Proto3) |> c_of_compound name
  (* Repeated fields cannot have a default *)
  | _, { label = Some Label.LABEL_REPEATED; default_value = Some _; _ } ->
  ...
  | _, { label = Some Label.LABEL_REPEATED; type' = Some Type.TYPE_MESSAGE; type_name; _ }
    ->
    let spec = spec_of_message ~scope type_name in
    Repeated (number, spec, Not_packed) |> c_of_compound name
  (* Repeated enum *)
  | ( _
    , { label = Some Label.LABEL_REPEATED
      ; type' = Some Type.TYPE_ENUM
      ; type_name
      ; options
      ; _
      } ) ->
    ...
    let packed =
      ...
      | `Proto3, _ -> Packed
    in
    Repeated (number, spec, packed) |> c_of_compound name
  (* Repeated basic type *)
...
let c_of_oneof ~params ~syntax:_ ~scope OneofDescriptorProto.{ name; _ } fields =
  ...
  let oneof =
    let oneof_elems =
      List.map
        ~f:(fun (index, name, type', Espec spec) ->
          let adt_name = Scope.get_name_exn scope name in
          ( adt_name
          , Oneof_elem (index, spec, (type', sprintf "fun v -> %s v" adt_name, "v", None))
          ))
        field_infos
    in
    let type' =
      field_infos
      |> List.map ~f:(fun (_, name, type', _) ->
...
let split_oneof_decl fields oneof_decls =
  ...
  let rec inner oneofs oneof_decls = function
    | ({ oneof_index = Some i; _ } as o1) :: fs ->
      (match oneofs with
      | [] -> inner [ o1 ] oneof_decls fs
      | { oneof_index = Some j; _ } :: _ when i = j -> inner (o1 :: oneofs) oneof_decls fs
      | oneofs ->
        `Oneof (List.hd oneof_decls, List.rev oneofs)
        :: inner [ o1 ] (List.tl oneof_decls) fs)
    | f :: fs ->
      (match oneofs with
      ...
      | oneofs ->
        `Oneof (List.hd oneof_decls, List.rev oneofs)
        :: `Field f
        :: inner [] (List.tl oneof_decls) fs)
    | [] ->
      (match oneofs, oneof_decls with
      | [], [] -> []
      | oneofs, [ oneof_decl ] -> [ `Oneof (oneof_decl, List.rev oneofs) ]
...
let make
  ...
  let constructor_arg = function
    | { name; type' = { modifier = Required; _ }; _ } ->
      sprintf "~%s" (Scope.get_name scope name)
    | { name; _ } -> sprintf "?%s" (Scope.get_name scope name)
  in
  let constructor_default_value : c -> string option =
   fun c ->
    let dv =
      match c with
      | { type' = { modifier = Optional | Required; _ }; _ } -> None
      | { name; type' = { modifier = List; _ }; _ } ->
        Some (Scope.get_name scope name, "[]")
      | { name; type' = { modifier = No_modifier default; _ }; _ } ->
        Some (Scope.get_name scope name, default)
    in
    Option.map
      ~f:(fun (name, default) ->
      ...
      dv
  in
  let prepend ?(cond = true) elm l =
    match cond with
    | true -> elm :: l
    | false -> l
  in
  let append ?(cond = true) elm l =
    match cond with
    | true -> l @ [ elm ]
    | false -> l
  in
  ...
  let type_constr fields =
    match fields, t_as_tuple with
    | [], _ -> "unit"
    | [ field ], true -> field
    | fields, true -> String.concat ~sep:" * " fields |> sprintf "(%s)"
    | fields, false -> String.concat ~sep:"; " fields |> sprintf "{ %s }"
  in
  let type_destr fields =
    match fields, t_as_tuple with
    | [], _ -> "()"
    | [ field ], true -> field
    | fields, true -> String.concat ~sep:", " fields |> sprintf "(%s)"
  ...
  let type' =
    List.rev_map
      ~f:(fun { name; type'; _ } -> Scope.get_name scope name, typestr_of_type type')
      ts
  ...
  let field_names = List.map ~f:(fun { name; _ } -> Scope.get_name scope name) ts in
  let args = String.concat ~sep:" " field_names in
  let fields = append ~cond:has_extensions "extensions'" field_names in
  let constructor =
    sprintf
      "fun %s %s -> %s"
      (if has_extensions then "extensions'" else "_extensions")
      args
      (type_destr fields)
  in
  let apply =
    sprintf
      "fun ~f:f' %s -> f' %s %s"
      (type_destr fields)
      (if has_extensions then "extensions'" else "[]")
      args
  in
  ...
  let serialize_spec =
    ...
    String.concat ~sep:" ^:: " (spec @ [ "nil" ]) |> sprintf "Runtime'.Serialize.C.( %s )"
  in
