...
module Type_tree = struct
  ...
  let map_enum EnumDescriptorProto.{ name; value = values; _ } =
    ...
    let enum_names =
      List.map
        ~f:(fun EnumValueDescriptorProto.{ name; _ } ->
  ...
  let map_service ServiceDescriptorProto.{ name; method' = methods; _ } =
    ...
    let service_names =
      List.map
        ~f:(fun MethodDescriptorProto.{ name; _ } ->
  ...
  let split_oneof_fields fields =
    ...
    let field_number_of_field = function
      | FieldDescriptorProto.{ oneof_index = None; _ } ->
        failwith "Only oneof fields here"
      | FieldDescriptorProto.{ oneof_index = Some number; _ } -> number
    in
    let fields =
      List.sort
        ~cmp:(fun a b -> compare (field_number_of_field a) (field_number_of_field b))
        fields
    in
  ...
  let rec map_message
        ...
        ~f:
          (fun acc -> function
            | FieldDescriptorProto.
                { type_name = Some type_name; type' = Some Type.TYPE_MESSAGE; _ } ->
              type_name :: acc
            | FieldDescriptorProto.
                { type_name = Some type_name; type' = Some Type.TYPE_ENUM; _ } ->
              type_name :: acc
            | _ -> acc)
        fields
    in
    ...
    let fields =
      ...
      let plain_fields, oneof_fields =
        List.partition
          ~f:(fun FieldDescriptorProto.{ oneof_index; _ } -> oneof_index = None)
          fields
      in
      let plain_fields =
        ...
        List.fold_left
          ~init:acc
          ~f:(fun acc OneofDescriptorProto.{ name; _ } ->
            Option.value_exn ~message:"Oneof names cannot be null" name :: acc)
          oneof_decls
      in
      let oneof_fields =
        split_oneof_fields oneof_fields |> List.map ~f:(List.map ~f:field_name)
      in
  ...
  let create_cyclic_map { module_name = _; types } =
    ...
    let is_cyclic map name =
      let rec inner name (seen : StringSet.t) =
        (* If a type has more than one depend, then the cyclic chain is broken, and we can
  ...
  (** Create a map: proto_name -> ocaml_name. Mapping is done in multiple passes to
      prioritize which mapping wins in case of name clashes *)
  let create_name_map ~standard_f ~mangle_f names =
    ...
    let standard_name_map =
      let inject ~f map =
        List.fold_left
          ~init:map
          ~f:(fun map (name, mangled_name, standard_name) ->
      ...
      in
      StringMap.empty
      |> inject ~f:(fun name mangled_name _standard_name ->
           String.equal mangled_name name)
      |> inject ~f:(fun _name mangled_name standard_name ->
           String.equal mangled_name standard_name)
      |> inject ~f:(fun name mangled_name _standard_name ->
           String.equal
             (String.lowercase_ascii mangled_name)
             (String.lowercase_ascii name))
      |> inject ~f:(fun _name mangled_name standard_name ->
           String.equal
             (String.lowercase_ascii mangled_name)
             (String.lowercase_ascii standard_name))
    in
    List.fold_left
      ~init:[]
      ~f:(fun names (proto_name, ocaml_name, _) ->
        let ocaml_name =
  ...
  let create_file_db ~mangle cyclic_map { module_name; types } =
    ...
    let rec traverse_types map path types =
      let map_type
        ...
        ~name_map
        path
        { name; types; fields = plain_fields, oneof_fields; enum_names; service_names; _ }
        =
        let ocaml_name =
        ...
        in
        let map =
          StringMap.add_uniq ~key:path ~data:{ module_name; ocaml_name; cyclic } map
        in
      ...
      List.fold_left
        ~init:map
        ~f:(fun map type_ -> map_type ~map ~name_map path type_)
        types
    in
  ...
  let create_db (files : FileDescriptorProto.t list) =
    let inner proto_file =
      ...
      let file_db =
        create_file_db ~mangle:(option_mangle_names proto_file) cyclic_map map
      in
      file_db
    in
...
let dump_type_map type_map =
  ...
  StringMap.iter
    ~f:(fun ~key ~data:{ module_name; ocaml_name; cyclic; _ } ->
...
let for_descriptor t FileDescriptorProto.{ name; package; _ } =
  ...
  let package_depth =
    Option.value_map
      ~default:0
      ~f:(fun p -> String.split_on_char ~sep:'.' p |> List.length)
      package
  in
...
let push : t -> string -> t =
 fun t name -> { t with proto_path = t.proto_path ^ "." ^ name }
;;
...
let get_scoped_name ?postfix t name =
  ...
  Option.value_map
    ~default:type_name
    ~f:(fun postfix -> type_name ^ "." ^ postfix)
    postfix
;;
...
let get_name t name =
  ...
  | Some { ocaml_name; _ } ->
    String.split_on_char ~sep:'.' ocaml_name |> List.rev |> List.hd
