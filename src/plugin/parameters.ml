...
let parse_option str =
  ...
  | n ->
    `Expr
      ( String.sub str ~pos:0 ~len:n
      , String.sub str ~pos:(n + 1) ~len:(String.length str - n - 1) )
...
let parse parameters =
  ...
  |> List.fold_left ~init:default ~f:(fun param option ->
         ...
         | `Expr ("fixed_as_int", (("true" | "false") as v)) ->
           { param with fixed_as_int = bool_of_string v }
         | `Expr ("int64_as_int", (("true" | "false") as v)) ->
           { param with int64_as_int = bool_of_string v }
         | `Expr ("int32_as_int", (("true" | "false") as v)) ->
           { param with int32_as_int = bool_of_string v }
         | `Expr ("singleton_record", (("true" | "false") as v)) ->
           { param with singleton_record = bool_of_string v }
...
let use_snakecase options =
  Option.bind ~f:(fun option -> Spec.Options.Ocaml_options.get option) options
