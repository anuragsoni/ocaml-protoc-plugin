...
(** Slightly overloaded name here. Its also used for all other types which would go into a
    module *)
type module' =
  { module_name : string
  ; signature : Code.t
...
let emit_enum_type
  ...
  ~params
  EnumDescriptorProto.
    { name; value = values; options = _; reserved_range = _; reserved_name = _ }
  : module'
  =
  ...
  Code.emit
    ...
    "type t = %s %s"
    (List.map
       ~f:(fun EnumValueDescriptorProto.{ name; _ } -> Scope.get_name_exn scope name)
       values
    |> String.concat ~sep:" | ")
  ...
  let _ =
    List.fold_left
      ~init:IntSet.empty
      ~f:(fun seen EnumValueDescriptorProto.{ name; number; _ } ->
        let idx =
          Option.value_exn ~message:"All enum descriptions must have a value" number
        in
...
let emit_service_type scope ServiceDescriptorProto.{ name; method' = methods; _ } =
  let emit_signature t scope MethodDescriptorProto.{ name; input_type; output_type; _ } =
...
let emit_extension ~scope ~params field =
  ...
  Code.emit
    implementation
    `None
    "let extensions' = Runtime'.Extensions.set (%s) (extendee.%s) t in"
    t.serialize_spec
    extendee_field;
...
(* Emit a message plus all its subtypes. Why is this not being called recursively, but
   rather calling sub functions which never returns *)
let rec emit_message
  ~params
  ~syntax
  scope
  DescriptorProto.
  ...
  let () =
    ...
    | Some _name ->
      ...
      let extension_ranges =
        extension_ranges
        |> List.map ~f:(function
             | DescriptorProto.ExtensionRange.{ start = Some start; end' = Some end'; _ }
               -> start, end'
      ...
      Code.emit
        implementation
        `None
        "let serialize = Runtime'.Serialize.serialize %s (spec) in"
        extension_ranges;
...
let rec wrap_packages ~params ~syntax scope message_type services = function
  | [] ->
    let { module_name = _; implementation; _ } =
      emit_message ~params ~syntax scope message_type
    in
    List.iter
      ~f:(fun service -> Code.append implementation (emit_service_type scope service))
      services;
    implementation
  | package :: packages ->
    let implementation = Code.init () in
    ...
    Code.append
      implementation
      (wrap_packages ~params ~syntax scope message_type services packages);
...
let parse_proto_file
  ...
  let out_name = Filename.remove_extension name |> sprintf "%s.ml" in
